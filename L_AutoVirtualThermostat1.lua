...
local HEATSP_SID = SETPOINT_SID .. "_Heat"
local HEATSP_VAR = "CurrentSetpoint"
local COOLSP_SID = SETPOINT_SID .. "_Cool"
local COOLSP_VAR = "CurrentSetpoint"
...
local function inArray( a, w ) 
    if a == nil or w == nil then return false end
    assert( type(a) == "table")
    local k,v
    for k,v in pairs(a) do
        if v == w then return true, k end
    end
end
...
local function deviceOnOff( targetDevice, state, vtDev )
    ...
    if targetId > 0 and luup.devices[targetId] ~= nil then
        ...
        elseif luup.device_supports_service("urn:upnp-org:serviceId:SwitchPower1", targetId) then
            ...
            L("Don't know how to control %2 target %1", targetId, targetDevice)
            return false
        end
...
local function checkSensors(dev)
    ...
    local coolSP = getVarNumeric( COOLSP_VAR, setpointTemp, dev, COOLSP_SID )
    local heatSP = getVarNumeric( HEATSP_VAR, setpointTemp, dev, HEATSP_SID )
...
function varChanged( dev, sid, var, oldVal, newVal )
    ...
    elseif sid == OPSTATE_SID and var == "ModeState" then
        updateDisplayStatus(luup.device)
    elseif inArray( { MYSID, TEMPSENS_SID, HEATSP_SID, COOLSP_SID }, sid ) then
        checkSensors(luup.device)
    else
...
function actionSetEnergyModeTarget( dev, newMode )
    ...
    if newMode ~= EMODE_ECO then
        luup.variable_set( HEATSP_SID, HEATSP_VAR, luup.variable_get( MYSID, "NormalHeatingSetpoint", dev ) or sysTemps.default, dev )
        luup.variable_set( COOLSP_SID, COOLSP_VAR, luup.variable_get( MYSID, "NormalCoolingSetpoint", dev ) or sysTemps.default, dev )
    else
        luup.variable_set( HEATSP_SID, HEATSP_VAR, luup.variable_get( MYSID, "EcoHeatingSetpoint", dev ) or sysTemps.default, dev )
        luup.variable_set( COOLSP_SID, COOLSP_VAR, luup.variable_get( MYSID, "EcoCoolingSetpoint", dev ) or sysTemps.default, dev )
    end
...
local function saveEnergyModeSetpoints( dev )
    ...
    if currEmode ~= EMODE_ECO then
        luup.variable_set( MYSID, "NormalHeatingSetpoint", luup.variable_get( HEATSP_SID, HEATSP_VAR, dev), dev )
        luup.variable_set( MYSID, "NormalCoolingSetpoint", luup.variable_get( COOLSP_SID, COOLSP_VAR, dev), dev )
    else
        luup.variable_set( MYSID, "EcoHeatingSetpoint", luup.variable_get( HEATSP_SID, HEATSP_VAR, dev), dev )
        luup.variable_set( MYSID, "EcoCoolingSetpoint", luup.variable_get( COOLSP_SID, COOLSP_VAR, dev), dev )
    end
end
...
function actionSetCurrentSetpoint( dev, newSP, whichSP )
    ...
    local heatSP = getVarNumeric( HEATSP_VAR, sysTemps.default, dev, HEATSP_SID )
    local coolSP = getVarNumeric( COOLSP_VAR, sysTemps.default, dev, COOLSP_SID )

    if whichSP == "DualHeatingCooling" then
        luup.variable_set( SETPOINT_SID, "CurrentSetpoint", newSP, dev )
        luup.variable_set( HEATSP_SID, HEATSP_VAR, newSP, dev )
        luup.variable_set( COOLSP_SID, COOLSP_VAR, newSP, dev )
    elseif whichSP == "Heating" then
        ...
        if newSP > coolSP then
            coolSP = newSP
            luup.variable_set( COOLSP_SID, COOLSP_VAR, coolSP, dev )
        end
        luup.variable_set( HEATSP_SID, HEATSP_VAR, newSP, dev )
    elseif whichSP == "Cooling" then
        ...
        if newSP < heatSP then
            heatSP = newSP
            luup.variable_set( HEATSP_SID, HEATSP_VAR, heatSP, dev )
        end
        luup.variable_set( COOLSP_SID, COOLSP_VAR, newSP, dev )
    end
...
local function plugin_runOnce(dev)
    ...
    if (rev == 0) then
        ...
        if luup.attr_get("TemperatureFormat",0) == "C" then
            luup.variable_set(HEATSP_SID, HEATSP_VAR, "18", dev)
            luup.variable_set(COOLSP_SID, COOLSP_VAR, "24", dev)
            ...
            luup.variable_set(HEATSP_SID, HEATSP_VAR, "64", dev)
            luup.variable_set(COOLSP_SID, COOLSP_VAR, "76", dev)
            luup.variable_set(MYSID, "Differential", "1", dev)
    ...
    if rev < 010100 then
        ...
        luup.variable_set( OPSTATE_SID, "ModeState", "Idle", dev )
        luup.variable_set( HEATSP_SID, HEATSP_VAR, luup.variable_get( MYSID, "SetpointHeating", dev ), dev )
        luup.variable_set( COOLSP_SID, COOLSP_VAR, luup.variable_get( MYSID, "SetpointCooling", dev ), dev )
        luup.variable_set( MYSID, "NormalHeatingSetpoint", luup.variable_get( HEATSP_SID, HEATSP_VAR, dev ), dev )
        luup.variable_set( MYSID, "NormalCoolingSetpoint", luup.variable_get( COOLSP_SID, COOLSP_VAR, dev ), dev )
        if luup.attr_get("TemperatureFormat",0) == "C" then
            luup.variable_set( MYSID, "EcoHeatingSetpoint", "13", dev )
...
function plugin_init(dev)
    ...
    luup.variable_watch( "avtVarChanged", HEATSP_SID, HEATSP_VAR, dev )
    luup.variable_watch( "avtVarChanged", COOLSP_SID, COOLSP_VAR, dev )
